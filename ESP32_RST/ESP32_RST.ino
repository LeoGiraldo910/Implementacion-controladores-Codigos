...
// Variables para la ecuación en diferencias del precompensador F
float previousA = 0;
float previousR = 0;
float currentA = 0;
float currentR = 0;
...
// Variables para la ecuación en diferencias del controlador C
float previousU = 0;
float previousY = 0;
float currentU = 0;
float currentY = 0;
...
void loop() {
  ...
  if (elapsedTime >= sampleTime * 1000) { // Convertir tiempo de muestreo a milisegundos
    ...
    // Implementar la ecuación en diferencias
      currentA = 0.067254 * currentR - 0.106664844 * previousR + 0.04304256 * previous2R + 1.87 * previousA - 0.8712 * previous2A ;

      // Actualizar los valores anteriores
      previous2R = previousR;
      previous2A = previousA;
      previousR = currentR;
      previousA = currentA;

    F_ref = currentA;

    float error = F_ref - angleVoltage;
    ...
    if (runningTime >= settlingTime) {
      ...
      // Implementar la ecuación en diferencias
      currentY = 0.00051634 * previousU + 0.00051685634 * previous2U + 1.87 * previousY - 0.8712 * previous2Y;

      // Actualizar los valores anteriores
      previous2U = previousU;
      previous2Y = previousY;
      previousU = currentU;
      previousY = currentY;

      // Mapear la señal de control al rango de velocidad del motor1
      int mappedSpeed = map((currentY-0.85) * 1000, 0, 3300, 1060, 1220); // Ajustar los límites si es necesario
